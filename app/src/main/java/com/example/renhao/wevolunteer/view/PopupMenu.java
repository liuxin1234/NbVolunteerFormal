...
public class PopupMenu extends PopupWindow implements OnClickListener {
    ...
    public PopupMenu(Activity activity, String[] tabs) {
        ...
        this.setWidth(dip2px(activity, 175));// 设置菜单的宽度（需要和菜单于右边距的距离搭配，可以自己调到合适的位置）
    ...
    public void onClick(View v) {
        ...
        if (v == v_item1) {
            ...
            str = tabs[0];
            TextView tvAboutUs = (TextView) v_item1.findViewById(R.id.tv_menu_all);
            tvAboutUs.setText(str);
        } else if (v == v_item2) {
            menuitem = MENUITEM.ITEM2;
            str = tabs[1];
            TextView tvAboutUs = (TextView) v_item2.findViewById(R.id.tv_menu_job);
            tvAboutUs.setText(str);
        } else if (v == v_item3) {
            menuitem = MENUITEM.ITEM3;
            str = tabs[2];
            TextView tvAboutUs = (TextView) v_item3.findViewById(R.id.tv_menu_activity);
            tvAboutUs.setText(str);
        } else if (v == v_item4) {
            menuitem = MENUITEM.ITEM4;
            str = tabs[3];
            TextView tvAboutUs = (TextView) v_item4.findViewById(R.id.tv_menu_center);
            tvAboutUs.setText(str);
        } else if (v == v_item5) {
            menuitem = MENUITEM.ITEM5;
            str = tabs[4];
            TextView tvAboutUs = (TextView) v_item5.findViewById(R.id.tv_menu_station);
            tvAboutUs.setText(str);
        }
